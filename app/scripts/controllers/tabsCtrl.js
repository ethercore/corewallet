...
var tabsCtrl = function($scope, globalService, $translate, $sce) {
  ...
  $scope.defaultNodeKey = 'ethercore';
  $scope.customNode = {
    options: 'eth',
    name: '',
    url: '',
  ...
  $scope.gasChanged = function() {
    ...
    $scope.gasPriceMsg = ethFuncs.gasAdjustment < 1 ? true : false;
  };
  var setGasValues = function() {
    $scope.gas = {
      curVal: 1,
      value: globalFuncs.localStorage.getItem(gasPriceKey, null) ? parseInt(globalFuncs.localStorage.getItem(gasPriceKey)) : 1,
      max: 99,
      min: 0,
      step: 1
    };
